...
template <typename T, int SubGroupSize> struct Rt64_128_PCIE {
  ...
  static inline bool recvMessages(message_t &messages, T* ptr, uint32_t flag) {
...
#endif
    if constexpr (SubGroupSize == 16)
      retry = (lid == 3 && messages[3] != flag)
        || (lid == 7 && messages[3] != flag)
        || (lid == 11 && messages[3] != flag)
        || (lid == 15 && messages[3] != flag);
    ...
    if constexpr (SubGroupSize == 32)
      retry = (lid == 3 && messages[3] != flag)
        ...
        || (lid == 11 && messages[3] != flag)
        || (lid == 15 && messages[3] != flag)
        || (lid == 19 && messages[3] != flag)
        || (lid == 23 && messages[3] != flag)
        || (lid == 27 && messages[3] != flag)
        || (lid == 31 && messages[3] != flag);
    return retry;
  }
...
#if defined(__SYCL_DEVICE_ONLY__) && defined(__SPIR__)
    using math_t = sycl::vec<T, sizeof(message_t)/sizeof(T)>;
    v = sycl::bit_cast<message_t>(
        sycl::bit_cast<math_t>(m) + sycl::bit_cast<math_t>(v)
    );
#endif
  }
};
